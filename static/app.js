...
async function autoTranscribe(){
 ...
 try{
  ...
  let queue=Promise.resolve(),completed=0,chunkIndex=0;const CHUNK_SECONDS=45;
  const sendChunk=async blob=>{
    ...
    completed++;
    $("transcribeHint").textContent="Расшифровано примерно "+Math.min(state.duration,completed*CHUNK_SECONDS).toFixed(0)+" из "+state.duration.toFixed(0)+" сек. · фрагмент "+completed;
  };
