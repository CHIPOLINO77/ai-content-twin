...
async function health(){
  try{
    ...
    $("apiStatus").textContent=d.polza_configured&&d.model_configured?"Polza AI подключён":"Backend online • настрой .env";
...
$("generate").onclick=async()=>{
  ...
  try{
    const r=await fetch(API+"/api/ideas",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({topic,count:10,platform:"shorts"})});
    const data=await r.json(); if(!r.ok) throw new Error(data.detail||"Generation failed");
    const list=Array.isArray(data)?data:[data];
    $("ideas").innerHTML=list.map((x,i)=>"<div class='idea'><h3>"+(i+1)+". "+esc(x.title||"Идея")+"</h3><p><b>Hook:</b> "+esc(x.hook||"—")+"</p><p>"+esc(x.angle||"")+"</p><span class='tag'>"+esc(x.why_it_fits||"AI Content Twin")+"</span></div>").join("");
